...
async def test_state_machine(mocker, repo_fixture, board_fixture, player1_fixture, player2_fixture, checker_fixture):
    ...
    with pytest.raises(StateValidationExceptions):
        await state_machine.handle_event(event=move_event, room_id=game.room_id)
    ...
    state_response = await state_machine.handle_event(event=start_event, room_id=game.room_id)
